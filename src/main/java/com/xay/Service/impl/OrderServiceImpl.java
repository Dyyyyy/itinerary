...
public class OrderServiceImpl implements OrderService{
    ...
    @Override
    public BaseResult<Object> startOrder(Integer orderId) {
        OrderDO orderDO = orderMapper.getOrderById(orderId);
        if (orderDO != null){
            if (orderDO.getStatus() >= 3){
                return new BaseResult<>(500, "Order has already been started");
            }else orderMapper.startOrder(orderId);
            return new BaseResult<>();
        }
        return new BaseResult<>(500, "No order found");
    }
    ...
    @Override
    public BaseResult<Object> cancelOrder(Integer orderId) {
        OrderDO orderDO = orderMapper.getOrderById(orderId);
        if (orderDO != null){
