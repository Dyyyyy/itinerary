...
public interface OrderMapper {
     ...
     * @return
     */
    @Update("UPDATE orders SET status=3, pay_time=CURRENT_TIMESTAMP WHERE order_id=#{order_id}")
    int startOrder(@Param("order_id")Integer order_id);
     ...
     * @return
     */
    @Update("UPDATE orders SET status=6, pay_time=CURRENT_TIMESTAMP WHERE order_id=#{order_id}")
     ...
     * @return
     */
    @Update("UPDATE orders SET status=4, deliver_time=CURRENT_TIMESTAMP WHERE order_id=#{order_id}")
     ...
     * @return
     */
    @Update("UPDATE orders SET status=5, deal_time=CURRENT_TIMESTAMP WHERE order_id=#{order_id}")
