...
function currentPage() {
    ...
    if (location.indexOf(main) != -1){
            ...
            success: function(data, statusText, xhr){
                if (data.code == 200){
                    ...
                    for (let i = 0; i < journeys.length; i++){
                        ...
                        let budget = $("<p></p>").text("PriceSpan:  " + journey.lowPrice + " ~ " + journey.highPrice);
        ...
        let journeyIds5 = [];
        let journeyIds6 = [];
        $("#canceled").children("li").remove();
        $("#toAccept").children("li").remove();
        $("#toPay").children("li").remove();
        $("#toStart").children("li").remove();
            ...
            success: function(data, statusText, xhr){
                if (data.code == 200){
                    ...
                    for (let i = 0; i < orders.length; i++){
                        journeyIds0.push(data.data[i].journeyId)
                    }
                    for (let i = 0; i < journeyIds0.length; i++){
                        $.ajax({
                            url: "journeys/id?journeyId=" + journeyIds0[i],
                            type: 'GET',
                            context: document.body,
                            success: function (data, statusText, xhr) {
                                if (data.code == 200) {
                                    let journey = data.data;
                                    let ele = $("<li class='info-element' value='" + journeyIds0[i] + "\' onclick='showOrder(this)' id=\'acc" + i + "\'></li>");
                                    let dest = $("<p></p>").text("Destination:  " + journey.cityName);
                                    let type = $("<p></p>").text("Type:  " + ((journey.tourType == 1)?t1:(journey.tourType == 2)?t2:t3));
                                    let tags = $("<p></p>").text("Tags:  " + journey.tags);
                                    let time = $("<p></p>").text("Time:  " + journey.startTime + "  TO  " + journey.endTime);
                                    let duration = $("<div class='agile-detail'></div>");
                                    let button = $("<button value='" + orders[i].orderId + "&" + journeyIds0[i] + "\' class='pull-right btn btn-xs btn-danger' onclick='changeToAccept(this)'></button>").text("Accept");
                    ...
                    for (let i = 0; i < journeyIds1.length; i++){
                        $.ajax({
                            ...
                            success: function (data, statusText, xhr) {
                                if (data.code == 200) {
                                    let journey = data.data;
                                    let ele = $("<li class='danger-element' value='" + journeyIds1[i] + "\' onclick='showOrder(this)' id=\'acc" + i + "\'></li>");
                                    let dest = $("<p></p>").text("Destination:  " + journey.cityName);
                                    let type = $("<p></p>").text("Type:  " + ((journey.tourType == 1)?t1:(journey.tourType == 2)?t2:t3));
                                    let tags = $("<p></p>").text("Tags:  " + journey.tags);
                                    let time = $("<p></p>").text("Time:  " + journey.startTime + "  TO  " + journey.endTime);
                                    let duration = $("<div class='agile-detail'></div>");
                                    let button = $("<button value='" + orders[i].orderId + "\' class='pull-right btn btn-xs btn-danger' onclick='changeToPay(this)'></button>").text("Pay");
                                    duration.text("Price:  " + journey.price + "RMB");
                                    ele.append(dest).append(type).append(tags).append(time).append(duration);
                                    $("#toPay").append(ele);
                                    if (userType == 0){
                        ...
                        $.ajax({
                            ...
                            success: function (data, statusText, xhr) {
                                if (data.code == 200) {
                                    ...
                                    let time = $("<p></p>").text("Time:  " + journey.startTime + "  TO  " + journey.endTime);
                                    let duration = $("<div class='agile-detail'></div>");
                                    let button = $("<button value='" + orders[i].orderId + "\' class='pull-right btn btn-xs btn-danger' onclick='changeToStart(this)'></button>").text("Start");
                                    let button2 = $("<button value='" + orders[i].orderId + "\' class='pull-right btn btn-xs btn-danger' onclick='changeToCancel(this)'></button>").text("Cancel");
                                    duration.text("Price:  " + journey.price + "RMB");
                                    ele.append(dest).append(type).append(tags).append(time).append(duration);
                                    $("#toStart").append(ele);
                                    if (userType == 0){ //导游
                    ...
                    for (let i = 0; i < journeyIds3.length; i++){
                        $.ajax({
                            ...
                            success: function (data, statusText, xhr) {
                                if (data.code == 200) {
                                    let journey = data.data;
                                    let ele = $("<li class='danger-element' value='" + journeyIds3[i] + "\' onclick='showOrder(this)' id=\'acc" + i + "\'></li>");
                                    let dest = $("<p></p>").text("Destination:  " + journey.cityName);
                                    let type = $("<p></p>").text("Type:  " + ((journey.tourType == 1)?t1:(journey.tourType == 2)?t2:t3));
                                    let tags = $("<p></p>").text("Tags:  " + journey.tags);
                                    let time = $("<p></p>").text("Time:  " + journey.startTime + "  TO  " + journey.endTime);
                    ...
                    for (let i = 0; i < journeyIds4.length; i++){
                        $.ajax({
                            ...
                            success: function (data, statusText, xhr) {
                                if (data.code == 200) {
                                    let journey = data.data;
                                    let ele = $("<li class='warning-element' value='" + journeyIds4[i] + "\' onclick='showOrder(this)' id=\'acc" + i + "\'></li>");
                                    let dest = $("<p></p>").text("Destination:  " + journey.cityName);
                                    let type = $("<p></p>").text("Type:  " + ((journey.tourType == 1)?t1:(journey.tourType == 2)?t2:t3));
                                    let tags = $("<p></p>").text("Tags:  " + journey.tags);
                                    let time = $("<p></p>").text("Time:  " + journey.startTime + "  TO  " + journey.endTime);
        ...
        $.ajax({
            ...
            success: function(data, statusText, xhr){
                if (data.code == 200){
                    let orders = data.data;
                    for (let i = 0; i < orders.length; i++){
                        journeyIds5.push(data.data[i].journeyId)
                    }
                    for (let i = 0; i < journeyIds5.length; i++){
                        $.ajax({
                            url: "journeys/id?journeyId=" + journeyIds5[i],
                            type: 'GET',
                            context: document.body,
                            success: function (data, statusText, xhr) {
                                if (data.code == 200) {
                                    let journey = data.data;
                                    let ele = $("<li class='success-element' value='" + journeyIds5[i] + "\' onclick='showOrder(this)' id=\'fin" + i + "\'></li>");
                                    let dest = $("<p></p>").text("Destination:  " + journey.cityName);
                                    let type = $("<p></p>").text("Type:  " + ((journey.tourType == 1)?t1:(journey.tourType == 2)?t2:t3));
                                    let tags = $("<p></p>").text("Tags:  " + journey.tags);
                                    let time = $("<p></p>").text("Time:  " + journey.startTime + "  TO  " + journey.endTime);
        ...
        $.ajax({
            ...
            success: function(data, statusText, xhr){
                if (data.code == 200){
                    let orders = data.data;
                    for (let i = 0; i < orders.length; i++){
                        journeyIds6.push(data.data[i].journeyId)
                    }
                    for (let i = 0; i < journeyIds6.length; i++){
                        $.ajax({
                            url: "journeys/id?journeyId=" + journeyIds6[i],
                            type: 'GET',
                            context: document.body,
                            success: function (data, statusText, xhr) {
                                if (data.code == 200) {
                                    let journey = data.data;
                                    let ele = $("<li class='success-element' value='" + journeyIds6[i] + "\' onclick='showOrder(this)' id=\'fin" + i + "\'></li>");
                                    let dest = $("<p></p>").text("Destination:  " + journey.cityName);
                                    let type = $("<p></p>").text("Type:  " + ((journey.tourType == 1)?t1:(journey.tourType == 2)?t2:t3));
                                    let tags = $("<p></p>").text("Tags:  " + journey.tags);
                                    let time = $("<p></p>").text("Time:  " + journey.startTime + "  TO  " + journey.endTime);
    ...
    }else if (location.indexOf(message) != -1){
        $.ajax({
            ...
            success: function (data, statusText, xhr) {
                if (data.code == 200) {
                    $("#contactList").children("li").remove();
                    let contacts = data.data;
                    for (let i in contacts){
                        let ele = $("<li onclick='chatBox(this)'><a>" + contacts[i] + "</a></li>");
...
$("#toggle1").click(function() {
    $("#toggle1").attr("class", "btn btn-danger");
    $("#toggle2").attr("class", "btn btn-white");
    $("#toggle3").attr("class", "btn btn-white");
    $("#unstarted").show();
    $("#unfinished").hide();
    $("#toFinished").hide();
});
...
$("#toggle2").click(function() {
    $("#toggle1").attr("class", "btn btn-white");
    $("#toggle2").attr("class", "btn btn-danger");
    $("#toggle3").attr("class", "btn btn-white");
    $("#unstarted").hide();
    $("#unfinished").show();
    $("#toFinished").hide();
...
$("#toggle3").click(function() {
    $("#toggle1").attr("class", "btn btn-white");
    $("#toggle2").attr("class", "btn btn-white");
    $("#toggle3").attr("class", "btn btn-danger");
    $("#unstarted").hide();
    $("#unfinished").hide();
    $("#toFinished").show();
});
        ...
        success: function(data, statusText, xhr){
            if (data.code == 200){
                $.ajax({
                    ...
                    success: function(data, statusText, xhr){
                        if (data.code == 200){
                            alert("Change to ToBePay successfully");
                        }
                    }
                });
...
function show_chatmodal(ele) {
    $('#ChatModal').modal('show');
}

$("#confirmChat").click(function () {
    $.ajax({
        url: "/messages",
        type: 'POST',
        data: JSON.stringify({
    ...
    let ele = $("<li onclick='chatBox(this)'><a>" + $("#toName").val() + "</a></li>");
    // let ele = $("<li onclick='chatBox(this)'><a>" + $("#toName").val() + "<span class='label label-danger pull-right'>16</span></a></li>");
    $("#contactList").append(ele);
});
    ...
    $.ajax({
        ...
        success: function(data, statusText, xhr){
            if (data.code == 200){
                ...
                $("#chatContent").append($("<p>" + date + "</p>")).append($("<p>" + text.val() + "</p>"));
                text.val("");
            }else alert(data.message);
        }
    });
...
function chatBox(ele) {
    chatName = $(ele).text();
    $("#chatContent").children("p").remove();
    $("#chatBulk").show();
    // $("#chatName").text($("#toName").val());
    ...
    $.ajax({
        url: "/images?username=" + $(ele).text() + "&type=" + iuserType,
        type: 'GET',
        context: document.body,
        success: function(data, statusText, xhr){
            if (data.code == 200){
                $("#chatImg").attr("src", "data:image/jpg;base64," + data.data);
            }else $("#chatImg").attr("src", "img/examples/2.jpg");
        }
    });
    $.ajax({
        url: "/messages/bilateral?usernameFrom=" + username + "&usernameTo=" + $(ele).text(),
        type: 'GET',
        context: document.body,
        success: function(data, statusText, xhr){
            if (data.code == 200){
                ...
                for(let i = 0; i < messages.length; i++) {
                    let ele = $("<p></p>").text(messages[i].messageContent);
                    let time = $("<p></p>").text(messages[i].time);
                    if (messages[i].messageContent !== ""){
                        $("#chatContent").append(time).append(ele);
                    }
                }
            }
        }
    });
    self.setInterval(checkNewMessage(chatName), 2000);
}

function checkNewMessage(str) {
    $.ajax({
        url: "/messages/bilateral?usernameFrom=" + username + "&usernameTo=" + str,
        type: 'GET',
        context: document.body,
        success: function(data, statusText, xhr){
            if (data.code == 200){
